 ...
 * Category SSG Page
 *
 * Generates static category pages with filtered job listings:
...
<head>
  ...
  <title>{{categoryTitle}} - Lowongan Kerja | Megawe</title>
  <meta name="description" content="{{description}}">
  <meta name="keywords" content="{{keywords}}">
  <meta name="robots" content="index, follow">
  <link rel="canonical" href="https://megawe.net/categories/{{slug}}">

  <!-- Open Graph -->
  <meta property="og:title" content="{{categoryTitle}} - Lowongan Kerja | Megawe">
  <meta property="og:description" content="{{description}}">
  <meta property="og:url" content="https://megawe.net/categories/{{slug}}">
  <meta property="og:type" content="article">
  <meta property="og:site_name" content="Megawe">

  <!-- Twitter Card -->
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="{{categoryTitle}} - Lowongan Kerja | Megawe">
...
<body class="bg-gray-50 dark:bg-gray-900">
  ...
  <!-- Hero Section -->
  <div class="bg-blue-600 dark:bg-blue-800 text-white py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <main class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Related Categories -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Kategori Terkait</h2>
        <div class="flex flex-wrap gap-3">
          {{#each relatedCategories}}
          <a href="/categories/{{slug}}" class="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
            {{name}} ({{count}})
          </a>
          {{/each}}
        </div>
...
/**
 * Generate Schema.org structured data for category page
 */
function generateCategorySchema(categoryData: {
  name: string;
  description: string;
  jobCount: number;
  jobs: any[];
  ...
  const schema = {
    ...
    "@type": "CollectionPage",
    "name": `${categoryData.name} - Lowongan Kerja`,
    "description": categoryData.description,
    "url": `https://megawe.net/categories/${categoryData.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    "mainEntity": {
      "@type": "ItemList",
      "numberOfItems": categoryData.jobCount,
...
/**
 * Convert database job to template format
 */
function jobToCategoryTemplate(job: DatabaseJob): Record<string, any> {
  return {
    id: job.id,
    title: job.title,
    company: job.company,
...
/**
 * Get category statistics and related data
 */
async function getCategoryStats(env: Env, categoryName: string) {
  try {
    // Get total jobs by category
    const totalQuery = `
      SELECT COUNT(*) as count FROM jobs
      WHERE category = ? AND is_active = 1
    `
    const totalResult = await env.MEGAWE_DB.prepare(totalQuery).bind(categoryName).first()

    // Get all categories for related suggestions
    ...
    const locationsQuery = `
      ...
      FROM jobs
      WHERE category = ? AND location IS NOT NULL AND is_active = 1
      GROUP BY location
      ORDER BY count DESC
      LIMIT 5
    `
    const locationsResult = await env.MEGAWE_DB.prepare(locationsQuery).bind(categoryName).all()

    return {
      totalJobs: totalResult?.count || 0,
      relatedCategories: (categoriesResult.results || []).map(row => ({
        name: row.category,
        count: row.count,
        slug: row.category.toLowerCase().replace(/[^a-z0-9]+/g, '-')
      })),
      topLocations: (locationsResult.results || []).map(row => row.location)
    }
  } catch (error) {
...
/**
 * Template rendering for category pages
 */
function renderCategoryTemplate(template: string, variables: Record<string, any>): string {
...
/**
 * Main Pages Function handler for category pages
 */
export async function onRequestGet(context: {
  ...
  try {
    ...
    if (!jobsResult.results || jobsResult.results.length === 0) {
      // Return 404 page for empty categories
      return new Response(
        `<!DOCTYPE html>
        <html lang="id">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Kategori Tidak Ditemukan - Megawe</title>
        ...
        <body class="bg-gray-50 min-h-screen flex items-center justify-center">
          <div class="text-center">
            ...
            <p class="text-xl text-gray-600 mb-8">Kategori tidak ditemukan atau tidak memiliki lowongan aktif</p>
    ...
    const categoryStats = await getCategoryStats(context.env, categoryName)

    const templateVars = {
      categoryTitle: `${categoryName} - Lowongan Kerja`,
      description: `Temukan semua lowongan kerja di kategori ${categoryName}. Platform terpercaya untuk mencari pekerjaan impian Anda.`,
      keywords: `${categoryName}, lowongan kerja, karir, pekerjaan, ${categoryStats.topLocations.join(', ')}`,
      slug: categorySlug,
      totalJobs: categoryStats.totalJobs,
      ...
      // Related data
      relatedCategories: categoryStats.relatedCategories.slice(0, 5),
      topLocations: categoryStats.topLocations,

      // Job listings
      jobs: jobs.map(jobToCategoryTemplate),
      hasMoreJobs: categoryStats.totalJobs > jobs.length,

      // Schema.org data
      jsonld: generateCategorySchema({
        name: categoryName,
        description: `Temukan semua lowongan kerja di kategori ${categoryName}. Platform terpercaya untuk mencari pekerjaan impian Anda.`,
        jobCount: categoryStats.totalJobs,
        jobs: jobs.slice(0, 10) // First 10 jobs for schema
      })
    }

    // Render HTML template
    const htmlContent = renderCategoryTemplate(CATEGORY_TEMPLATE, templateVars)
